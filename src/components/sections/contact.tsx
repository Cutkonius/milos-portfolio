...
function WaysToWork({
  ...
  return (
    <div className="mt-11 w-full max-w-[780px] sm:mt-13">
      <div className="flex flex-col items-start gap-3 border-b border-amber/34 pb-4 sm:flex-row sm:items-end sm:justify-between sm:gap-5">
        <div>
          ...
          <div className="mt-1.5 text-[11px] font-semibold uppercase tracking-[0.09em] text-amber-soft">
            {String(total).padStart(2, "0")} / {totalMeta}
          </div>
        </div>
        <span className="max-w-[330px] text-[12px] leading-[1.5] text-text/72 sm:text-right">{note}</span>
      </div>

      <div className="border-b border-text/18">
        {receipts.map((receipt, index) => (
          <Reveal key={receipt.id} delay={index * 0.05}>
            <div className="grid grid-cols-[48px_1fr] gap-x-4 border-t border-text/16 py-5 first:border-t-0 sm:grid-cols-[58px_1fr_0.75fr] sm:items-center sm:gap-x-6 sm:py-6">
              <span className="text-[13px] font-semibold tracking-[0.09em] text-amber">
                {receipt.icon || `0${index + 1}`}
              </span>
...
export function Contact({ data }: { data: ContactSection }) {
  ...
  return (
    <section
      id="contact"
      className="editorial-section deco-section relative min-h-[820px] overflow-hidden bg-[#050713] px-5 pb-0 pt-[92px] sm:min-h-[880px] sm:px-8 sm:pt-[126px] lg:px-12"
    >
      ...
      <div className="relative mx-auto flex min-h-[728px] max-w-[1280px] flex-col sm:min-h-[754px]">
        <div className="max-w-[720px]">
          ...
          <WaysToWork
            ...
            total={data.nightTotal}
            totalMeta={data.nightTotalMeta}
          />
        </div>

        <footer className="mt-auto border-t border-text/18 pb-[34px] pt-[24px] text-[12.5px] text-text/72">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <span className="flex gap-6">
              ...
              <span className="text-text/54">{data.footerLockLabel}</span>
            </span>
          </div>
        </footer>
      </div>
