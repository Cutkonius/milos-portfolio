...
export function Work({ data }: { data: WorkSection }) {
  ...
  const image = caseStudyReady && featured.screenshot ? (
    <div className="group relative h-[300px] overflow-hidden border-y border-text/16 sm:h-[570px] lg:h-[690px]">
      ...
      <span
        aria-hidden="true"
        className="absolute bottom-0 right-0 border-l border-t border-text/20 bg-[#050713] px-4 py-3 text-[12px] font-semibold uppercase tracking-[0.09em] text-text/74 sm:px-6"
      >
  ...
  return (
    <section
      id="work"
      className="editorial-section deco-section scroll-mt-20 overflow-hidden bg-[#050713] px-5 pb-[110px] pt-[120px] sm:px-8 sm:pb-[150px] sm:pt-[155px] lg:px-12"
    >
      <div className="mx-auto max-w-[1280px]">
        <Reveal>
          <div className="flex flex-col items-start gap-3 border-b border-text/16 pb-5 sm:flex-row sm:items-end sm:justify-between sm:gap-6">
            ...
            <span className="max-w-[480px] text-[12px] font-semibold uppercase tracking-[0.095em] text-text/72 sm:text-right">
              {data.sublabel}
            </span>
          </div>
        </Reveal>
        ...
        <div className="grid gap-8 pb-10 pt-10 lg:grid-cols-[0.34fr_1fr] lg:gap-14 lg:pb-14 lg:pt-14">
          <Reveal delay={0.03}>
            <div className="flex items-baseline justify-between border-t border-amber/34 pt-3 text-[12px] font-semibold uppercase tracking-[0.095em] text-text/70 lg:block">
          ...
          <div>
            <Reveal delay={0.05}>
              <div className="flex flex-wrap items-end justify-between gap-x-8 gap-y-4">
                <h2 className="display-heading uppercase text-[clamp(62px,10vw,146px)] text-text">
                  {featured.title}
                </h2>
                <span className="pb-2 text-[12px] font-semibold uppercase tracking-[0.095em] text-amber">
            ...
            <Reveal delay={0.12}>
                <dl className="mt-10 grid border-y border-blue/34 sm:grid-cols-4">
                {[
                  ["Year", "2026"],
                  ...
                  <div
                    key={term}
                    className="grid grid-cols-[110px_1fr] border-b border-blue/24 py-4 last:border-b-0 sm:block sm:border-b-0 sm:border-r sm:px-4 sm:first:pl-0 sm:last:border-r-0"
                  >
        ...
        {!caseStudyReady && (progressStatement || progressBlocks.length > 0) && (
          <div>
            {progressStatement && (
              <Reveal delay={0.05}>
                <div className="grid gap-6 border-y border-text/16 py-8 sm:py-10 lg:grid-cols-[0.34fr_1fr] lg:gap-14 lg:py-12">
                  <span className="text-[12px] font-semibold uppercase tracking-[0.09em] text-amber-soft">
                    Project direction
                  </span>
                  <h3 className="max-w-[820px] text-[clamp(32px,5vw,66px)] font-semibold leading-[1] tracking-[-0.032em] text-text [text-wrap:balance]">
                    {progressStatement}
                  </h3>
                </div>
              </Reveal>
            ...
            {progressBlocks.length > 0 && (
              <div className="production-dossier border-b border-text/18">
                {progressBlocks.map((block, index) => (
                  <Reveal
                    key={block.id}
                    delay={0.04 + index * 0.04}
                    className={index < progressBlocks.length - 1 ? "border-b border-text/16" : ""}
                  >
                    <article className="group grid gap-5 py-7 sm:grid-cols-[70px_0.42fr_0.58fr] sm:items-start sm:gap-8 sm:py-9">
                      <span className="text-[12px] font-semibold tracking-[0.09em] text-amber transition-transform duration-500 group-hover:translate-x-1">
                        0{index + 1}
                      </span>
                      <h4 className="text-[13px] font-semibold uppercase tracking-[0.095em] text-text/88 transition-transform duration-500 group-hover:translate-x-1">
                        {block.heading}
                      </h4>
                      <p className="max-w-[520px] text-[15.5px] leading-[1.68] text-text/76 [text-wrap:pretty] sm:text-[16px]">
                        {block.body}
                      </p>
                    </article>
                  </Reveal>
